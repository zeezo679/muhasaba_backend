...
public class LogDeepWorkCommandHandler(IAppDbContext dbContext)
    : IRequestHandler<LogDeepWorkCommand, ErrorOr<Updated>>
{
    public async Task<ErrorOr<Updated>> Handle(LogDeepWorkCommand request, CancellationToken ct)
    {
    ...
    private async Task<ErrorOr<DailyLog>> FetchOrCreateDailyLog(
        Guid userId, DateOnly today, int deepWorkHours, CancellationToken ct)
    {
        ...
        if (dailyLog is null)
        {
            var createResult = DailyLog.Create(userId, today, deepWorkHours: deepWorkHours);
        ...
        {
            dailyLog.Update(deepWorkHours: deepWorkHours);
        }
