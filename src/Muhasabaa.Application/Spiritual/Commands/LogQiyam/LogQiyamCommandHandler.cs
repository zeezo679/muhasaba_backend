...
public sealed class LogQiyamCommandHandler(
    IAppDbContext dbContext,
    UserManager<ApplicationUser> userManager,
    IDailyLogService dailyLogService)
    : IRequestHandler<LogQiyamCommand, ErrorOr<Updated>>
{
    public async Task<ErrorOr<Updated>> Handle(LogQiyamCommand request, CancellationToken ct)
    {
        var today = DateOnly.FromDateTime(DateTime.UtcNow);

        var dailyLogResult = await FetchOrCreateDailyLog(request.UserId, today, request.PrayedQiyam, ct);
    ...
    private async Task<ErrorOr<DailyLog>> FetchOrCreateDailyLog(
        Guid userId, DateOnly today, bool prayedQiyam, CancellationToken ct)
    {
        ...
        if (dailyLog is null)
        {
            var createResult = DailyLog.Create(userId, today, prayedQiyam: prayedQiyam);
        ...
        {
            dailyLog.Update(prayedQiyam: prayedQiyam);
        }
